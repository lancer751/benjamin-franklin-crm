...
app.onError((err, c) => {
  if (err instanceof AppError){
   return c.json<ErrorResponse>(
      { success: false, error: err.message, isFormError: err.isFormError },
      err.status,
    );
  }
  ...
  if (mappedPrismaError) {
    return c.json<ErrorResponse>(
      { success: false, error: mappedPrismaError.message, isFormError: mappedPrismaError.isFormError },
      mappedPrismaError.status,
    );
  }
